...
async def main(url):
    async with async_playwright() as p:
        ...
        try:
            ...
            print("Looking for 'Registrieren' link to switch to registration form...")
            # The default view is Login. We need to switch to Sign up.
            await page.get_by_role("button", name="Registrieren", exact=True).nth(1).click()

            print("Filling out sign up form...")
            await page.get_by_label("Vollständiger Name").fill(full_name)
            await page.get_by_label("E-Mail-Adresse").fill(email)
            await page.get_by_label("Passwort").fill(password)

            print("Clicking the main 'Registrieren' button to submit...")
            await page.get_by_role("button", name="Registrieren", exact=True).first.click()
            ...
            # We will wait for the 'Anmelden' button to be visible as a confirmation.
            await page.wait_for_selector('button:has-text("Anmelden")', timeout=10000)
            ...
            # Look for an element that only appears when logged in, e.g., the 'Abmelden' (Logout) button.
            await page.wait_for_selector('button:has-text("Abmelden")', timeout=10000)
